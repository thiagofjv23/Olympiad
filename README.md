...
- **Atletas** — seletor por país; lista os atletas daquele país (nome, idade e Força), com clique para expandir e ver todos os atributos (incluindo o **trio favorito** — esporte, **modalidade** e **evento** em que compete — e o **Clube atual**, ou "Agente livre"). Com muitos atletas, a lista é exibida **em lotes** (botão **"Carregar mais"**) e há uma **busca** por nome ou clube — para a tela seguir fluida mesmo com milhares de atletas.
...
- **`continents.js`** — entidade **Continentes** (só lógica, sem UI). Raiz **Mundo** (`WORLD`) e os **6 continentes** (América do Sul; América do Norte, Central e Caribe; África; Ásia; Oceania; Europa), completando a hierarquia **Mundo → Continente → País → Região → Estado → Cidade**.
- **`countries.js`** — entidade **Países**. Cada país tem `id` (código do COI), `nome`, `iocCode`, `população`, um rating de **Força Olímpica** (0–100) e o **`continentId`**. Database inicial de **55 países** distribuídos pelos 6 continentes. Só a entidade País é modelada para todos; **cidades, clubes e atletas seguem só onde já existem** (o **Brasil**) — os demais são apenas o país + atributos.
- **`championships.js`** — entidade **Campeonatos/Torneios** (dados + helpers). Cada torneio tem `id`, `nome`, `país`, **`categoria`** (`categoryId`), **`coverage`** (o que disputa), **`format`** (como roda), **`scope`** (trava geográfica), `events` (provas resolvidas da coverage) e `stages` (cada etapa com `{number, date, events}`). O esporte é **derivado** da coverage (`getChampionshipSports`).
- **`tournaments.js`** — **gerador universal de torneios**, MODULAR em três eixos: **coverage** (evento/modalidade/esporte/vários/**todos** — Olimpíadas), **format** (etapa única, **liga**, **multidia**, **mata-mata**) e **scope** (regional/estadual/nacional/… escolhível). `buildTournament(config)` cria qualquer torneio; gera o **calendário do Brasil** (16 torneios de Atletismo abrangendo **todas as modalidades/eventos**, formato liga). Formatos inspirados em torneios reais (Diamond League, programa olímpico/Mundial, chaves de tênis — ver docs).
  - **Calendário do Brasil** (16 torneios): **Nacional** (`CNA-2026`, 2º sábado), **10 Estaduais** (1º sábado) e **5 Regionais** (3º sábado), todos Atletismo/todas as provas. Cada um tem `scope` que **trava a inscrição** (ver `eligibility.js`).
