...
- **`events.js`** — entidade **Eventos**: as **provas** de uma modalidade (Modalidade → Evento). Database com **~190 eventos** seguindo o **calendário olímpico** (uma leva por modalidade). Por ora guardam só a estrutura (id/nome/modalidade); o **modelo de resultado** (forma de resolução + desempenho) existe só onde já há modelo — hoje os **100 m rasos** (`EVT-ATL-100M`, Velocidade/Atletismo), com o cálculo de tempo (Força − fadiga → tempo, recorde 9,58 s como piso). Os demais recebem o modelo com a mecânica de ResultSystem (ver `TODO.md`). Exibidos na aba **Esportes**.
