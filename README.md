...
- **`events.js`** — entidade **Eventos**: as **provas** de uma modalidade (Modalidade → Evento). Database com **~190 eventos** seguindo o **calendário olímpico** (uma leva por modalidade). Cada evento diz qual **sistema de resultado** o resolve (`resultSystem`) e traz os **parâmetros** da sua prova; o `events.js` só **despacha** para o sistema certo. Hoje os **100 m** (`EVT-ATL-100M`, Velocidade/Atletismo) já são disputáveis (via `TimeResultSystem`). Os demais viram disputáveis à medida que forem parametrizados. Exibidos na aba **Esportes**.
